Shader "Custom/DH/FogOfWar"
{
    Properties
    {
        ...
        _VisibilitySoftness ("Visibility Softness", Range(0.0, 0.5)) = 0.18
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 frag(Varyings input) : SV_Target
            {
                ...
                float visibility = SAMPLE_TEXTURE2D(_FogVisibilityTex, sampler_FogVisibilityTex, gridUV).r;
                float visibilitySoftness = max(_VisibilitySoftness, 0.0001);
                visibility = smoothstep(0.5 - visibilitySoftness, 0.5 + visibilitySoftness, visibility);
                float cellFog = 1.0 - visibility;
                if (cellFog < 0.001)
                    return sceneColor;
