Shader "Custom/DH/FogOfWar_PointSoftEdge"
{
    Properties
    {
        ...
        _TrimStrength ("Trim Strength", Range(0.0, 1.0)) = 0.65
    ...
    SubShader
    {
        ...
        Pass
        {
            Name "DHFogOfWarPointSoftEdgePass"
            ...
            float SampleVisibilityPointRaw(float2 uv)
            {
                return SAMPLE_TEXTURE2D(_FogVisibilityTex, sampler_FogVisibilityTex, saturate(uv)).r;
            }

            float SampleVisibilityPointBinary(float2 uv)
            {
                return SampleVisibilityPointRaw(uv) >= 0.75 ? 1.0 : 0.0;
            }
            ...
            half4 frag(Varyings input) : SV_Target
            {
                ...
                float2 local = frac(gridCoord);
                float edgeSoftness = max(_EdgeSoftness, 0.0001);

                float currentVisibilityRaw = SampleVisibilityPointRaw(gridUV);
                float currentVisibleBinary = currentVisibilityRaw >= 0.75 ? 1.0 : 0.0;
                float leftVisibleBinary = SampleVisibilityPointBinary(gridUV + float2(-texelSize.x, 0.0));
                float rightVisibleBinary = SampleVisibilityPointBinary(gridUV + float2(texelSize.x, 0.0));
                float downVisibleBinary = SampleVisibilityPointBinary(gridUV + float2(0.0, -texelSize.y));
                float upVisibleBinary = SampleVisibilityPointBinary(gridUV + float2(0.0, texelSize.y));

                float visibleMask = currentVisibilityRaw;
                float leftTrim = (1.0 - leftVisibleBinary) * (1.0 - smoothstep(0.0, edgeSoftness, local.x));
                float rightTrim = (1.0 - rightVisibleBinary) * smoothstep(1.0 - edgeSoftness, 1.0, local.x);
                float downTrim = (1.0 - downVisibleBinary) * (1.0 - smoothstep(0.0, edgeSoftness, local.y));
                float upTrim = (1.0 - upVisibleBinary) * smoothstep(1.0 - edgeSoftness, 1.0, local.y);

                float trim = saturate(max(max(leftTrim, rightTrim), max(downTrim, upTrim)));
                if (currentVisibleBinary > 0.5)
                {
                    // Only erode inward from edges that are adjacent to fog,
                    // so we get a single "trimmed" visible shape instead of
                    // a bright outline plus an inner fog line.
                    visibleMask *= (1.0 - trim * _TrimStrength);
                }
                float cellFog = 1.0 - saturate(visibleMask);
                if (cellFog < 0.001)
                    return sceneColor;
