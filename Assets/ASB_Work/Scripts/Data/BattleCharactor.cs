...
public class BattleCharactor : MonoBehaviour, IUnitIdentifier
{
    ...
    private bool hasLoggedEmptySkillNameWarning;
    ...
    private void OnValidate()
    {
        ...
        SkillManager editorSkillManager = SkillManager.Instance;
        if (editorSkillManager == null)
        {
            editorSkillManager = Object.FindObjectOfType<SkillManager>();
            if (editorSkillManager == null)
            {
                TryEditorRefreshWeaponAndStats();
                return;
            }
        }

        RefreshAvailableSkillsForInspector();
        ResolveSelectedSkill(false);
        TryEditorRefreshWeaponAndStats();
    }

    /// <summary>에디터 OnValidate: WeaponManager가 씬에 있을 때만 무기 목록을 갱신합니다(없으면 목록을 비우지 않고 스킵).</summary>
    private void TryEditorRefreshWeaponAndStats()
    {
    ...
    public void RefreshAvailableSkillsForInspector()
    {
        ...
        {
            if (!hasLoggedEmptySkillNameWarning)
            {
                Debug.LogWarning($"[BattleCharactor] unitName이 비어 있어 클래스 스킬 매칭을 건너뜁니다: {name}");
                hasLoggedEmptySkillNameWarning = true;
            }
            return;
        }
        hasLoggedEmptySkillNameWarning = false;

        // TODO: 현재는 unitName/UnitData.Name 기반 임시 매칭
        // 추후 classKey / unitType 혹은 전용 식별자 컬럼 도입 시 이 부분 교체 예정
        if (SkillManager.Instance != null)
        {
            availableSkills.AddRange(SkillManager.Instance.GetAvailableSkillsForCharacter(unitName, level));
            return;
        }

        // SkillManager가 없는 개발 환경 fallback
        availableSkills.AddRange(skillDataLoader.GetSkillsByClass(unitName));
    }

    /// <summary>WeaponManager를 통해 unitName과 weaponClass가 일치하는 무기만 후보로 채웁니다. 매니저가 없으면 목록을 건드리지 않습니다(에디터에서 빈 목록으로 덮어쓰기 방지).</summary>
    public void RefreshAvailableWeapons()
    {
        ...
        availableWeapons.Clear();
        availableWeapons.AddRange(wm.GetWeaponsForClass(unitName));
    }
    ...
    public void ResolveEquippedWeapon(bool emitWarning = true)
    {
        if (availableWeapons != null && availableWeapons.Count > 0)
        {
            ...
            EquippedWeaponData = availableWeapons[localIndex];
            return;
        }
    ...
    public void ResolveSelectedSkill(bool emitWarning = true)
    {
        if (availableSkills != null && availableSkills.Count > 0)
        {
            ...
            SelectedSkillData = availableSkills[localIndex];
            return;
        }
